...
public class CompletedShiftTest {
    ...
    @Test
    public void givenPopulatedShiftWithOneAssignedEngineerShouldFailedToEvaluate() {
        val dailyShiftRepository = new DailyShiftRepository();
        val dailyShifts = dailyShiftRepository.getNextTwoWeekShift();
        val engineer = new Engineer(42, "Test Engineer");

        val randomIndex = ThreadLocalRandom.current().nextInt(0, dailyShifts.size());
    ...
    @Test
    public void givenPopulatedShiftWithTwoAssignedEngineerShouldSuccessToEvaluate() {
        val dailyShiftRepository = new DailyShiftRepository();
        val dailyShifts = dailyShiftRepository.getNextTwoWeekShift();
        val engineer = new Engineer(42, "Test Engineer");

        val firstRandomIndex = 3;
        val firstRandomShift = dailyShifts.get(firstRandomIndex);
        dailyShifts.add(firstRandomIndex, new DailyShift(firstRandomShift.getDate(), engineer, null));

        val secondRandomIndex = 5;
        val secondRandomShift = dailyShifts.get(firstRandomIndex);
        dailyShifts.add(secondRandomIndex, new DailyShift(secondRandomShift.getDate(), null, engineer));
